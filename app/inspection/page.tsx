...
async function fetchInspectionPage(schoolId: string): Promise<InspectionPageResponse | null> {
  const raw = localStorage.getItem("user");
  const token = raw ? JSON.parse(raw).access_token : null;

  const res = await fetch(`${API_BASE_URL}/school/inspection/${schoolId}`, {
    method: "GET",
    headers: {
...
export default function InspectionPage() {
  ...
  useEffect(() => {
    ...
    fetchInspectionPage(schoolId)
      .then((data) => {
        if (data === null) setNoInspection(true);
        else setData(data);
      })
    ...
    return (
      <DashboardLayout>
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <h2 className="text-lg font-semibold text-neutral-700 mb-1">No Inspection Scheduled</h2>
          <p className="text-sm text-neutral-400 max-w-sm">
            Your school has not been assigned an inspection yet. Once the admin schedules one, all details will appear here.
          </p>
        </div>
      </DashboardLayout>
    );
