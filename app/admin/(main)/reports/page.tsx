...
import { BarChart3, Download, ChevronDown, Calendar, Filter, TrendingUp, ArrowUpRight, MapPin, Building, CheckCircle, PieChart as PieChartIcon, Info } from "lucide-react";
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm flex flex-col relative group">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <div className="flex items-center gap-2">
                                    <h3 className="font-bold text-slate-800 tracking-tight">District-wise Coverage</h3>
                                    <Info className="w-3.5 h-3.5 text-slate-400" />
                                </div>
                                <p className="text-[11px] font-medium text-slate-500 mt-0.5 leading-relaxed pr-8">
                                    Percentage of total eligible schools that have successfully completed the recognition and mapping process in each district.
                                </p>
                            </div>
                        </div>
                        <button className="p-2 hover:bg-slate-50 rounded-lg text-slate-400 shrink-0"><ChevronDown className="w-4 h-4" /></button>
                    </div>
                ...
                <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm flex flex-col relative group">
                    <div className="flex items-start justify-between mb-2">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <div className="flex items-center gap-2">
                                    <h3 className="font-bold text-slate-800 tracking-tight">Recognition Trends</h3>
                                    <Info className="w-3.5 h-3.5 text-slate-400" />
                                </div>
                                <p className="text-[11px] font-medium text-slate-500 mt-0.5 leading-relaxed pr-2">
                                    Monthly breakdown comparing the volume of newly registered applications versus successfully approved schools over the last 6 months.
                                </p>
                            </div>
                        </div>
                        <div className="flex flex-col gap-1.5 shrink-0 mt-1">
                            <span className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400"><div className="w-2 h-2 rounded-full bg-blue-600" /> New Apps</span>
                            <span className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400"><div className="w-2 h-2 rounded-full bg-emerald-500" /> Approved</span>
                        </div>
                    </div>
                ...
                <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm relative group">
                    <div className="flex items-start gap-3 mb-8">
                        ...
                        <div>
                            <div className="flex items-center gap-2">
                                <h3 className="font-bold text-slate-800 tracking-tight">Core Infrastructure Availability</h3>
                                <Info className="w-3.5 h-3.5 text-slate-400" />
                            </div>
                            <p className="text-[11px] font-medium text-slate-500 mt-0.5 leading-relaxed pr-8">
                                Percentage of schools across the state that are currently equipped with these mandatory facilities. Lower percentages highlight critical gaps requiring immediate policy intervention.
                            </p>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-2 gap-6">
                    <div className="bg-slate-900 p-6 rounded-3xl text-white flex flex-col justify-between overflow-hidden relative group">
                        <div className="relative z-10">
                            <PieChartIcon className="w-8 h-8 text-blue-400 mb-4" />
                            <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Rural vs Urban</h4>
                            <p className="text-[10px] text-slate-500 font-medium mt-1 mb-2 leading-relaxed">Distribution of recognized schools across localities.</p>
                            <h3 className="text-2xl font-black mt-2">64% <span className="text-sm font-bold text-slate-500">/ 36%</span></h3>
                        </div>
                        <div className="mt-8 flex gap-1 h-1.5 overflow-hidden rounded-full"><div className="bg-blue-500 w-[64%]" /><div className="bg-white/10 w-[36%]" /></div>
                        <div className="absolute -right-8 -top-8 w-24 h-24 bg-blue-600/20 blur-[40px]" />
                    </div>
                    <div className="bg-blue-600 p-6 rounded-3xl text-white flex flex-col justify-between overflow-hidden relative group">
                        <div className="relative z-10">
                            <CheckCircle className="w-8 h-8 text-white/50 mb-4" />
                            <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-white/50">Recognition Rate</h4>
                            <p className="text-[10px] text-blue-200 font-medium mt-1 mb-2 leading-relaxed">Percentage of applications that successfully passed all SI and physical inspections.</p>
                            <h3 className="text-2xl font-black mt-2">12.4%</h3>
                        </div>
                        <p className="text-[10px] text-white/70 font-bold mt-4 italic">Based on latest audit cycle</p>
                        <div className="absolute -right-8 -bottom-8 w-32 h-32 bg-white/10 blur-[50px]" />
                    </div>
